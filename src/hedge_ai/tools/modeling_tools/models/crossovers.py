...
class Crossover:
    ...
    def __init__(self, tickers: List[str], crossover_config: CrossoverConfig, database_config: DatabaseCrossoversConfig) -> None:
    ...
    def _download_data(self) -> None:
        ...
        for ticker in self.tickers:
            df = fetcher.get_stock_data(ticker, start_date=self.crossover_config.start_date, end_date=self.crossover_config.end_date)
    ...
    def find(self, frame: pd.DataFrame) -> pd.DataFrame:
        ...
        pattern_counts = {"gain": 1, "loss": 1}  # Start from 1 since 0 means no pattern
        pattern_state = {"in_pattern": False, "start_index": None, "initial_price": None}
        ...
        for i in range(1, len(frame) - 1):
            ...
            conditions = {
                ...
                "rsi_overbought": frame["RSI_under_bought"].iloc[i]
            }
            logger.debug(f"Index {i} conditions: {conditions}")

            # Check pattern entry conditions
            if (not pattern_state["in_pattern"] and
                conditions["previous_bearish"] and
                conditions["current_bearish"] and
                conditions["crossunder"] and
                ...
                frame.loc[frame.index[pattern_slice], "gain"] = pattern_counts["gain"]
                frame.loc[frame.index[pattern_slice], "loss"] = pattern_counts["loss"]
                continue
                ...
                if (pattern_days == self.crossover_config.crossover_length or
                    abs(price_change) >= max(self.crossover_config.take_profit, self.crossover_config.stop_loss)):
                    
                    # Determine final pattern type and increment counter
                    pattern_type = "gain" if price_change > 0 else "loss"
                    pattern_counts[pattern_type] += 1
                    
                    logger.info(f"Pattern completed at index {i}: type={pattern_type}, "
        ...
        total_patterns = sum(count - 1 for count in pattern_counts.values())
        logger.info(f"Analysis complete: {total_patterns} total patterns "
                   f"({pattern_counts['gain']-1} gains, {pattern_counts['loss']-1} losses)")
        ...
        try:
            for ticker in self.tickers:
                ...
                results = {
                    ...
                    "count_crossovers": self.count_crossovers(df),
    ...
    def calculate_bearish_periods(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        """Calculate the length and details of each bearish period.
        A bearish period is when SMA_lower_below_upper is True."""
        logger.info("Calculating bearish periods")
        bearish_periods = []
        current_period = None
        all_uptrend = True

        for i in range(len(df)):
            is_bearish = df["SMA_lower_below_upper"].iloc[i]

            # Start a new bearish period
            if is_bearish and current_period is None:
                current_period = self._start_bearish_period(i, df)
                logger.debug(f"Started new bearish period at index {i}")

            # End current bearish period
            elif not is_bearish and current_period is not None:
                period_data = self._end_bearish_period(df, current_period, i - 1)  # End at previous index
        ...
        if current_period is not None:
            period_data = self._handle_final_period(df, current_period)
            all_uptrend &= period_data["is_uptrend"]
            bearish_periods.append(period_data)

        logger.info(f"Found {len(bearish_periods)} bearish periods")
    ...
    def count_crossovers(self, df: pd.DataFrame) -> Dict[str, int]:
        """Count the number of times SMAs cross over each other."""
        crossovers = {"bullish": 0, "bearish": 0, "total": 0}
    ...
    def _generate_outputs(self, df: pd.DataFrame, ticker: str, bearish_periods: List[Dict[str, Any]], metrics: Dict[str, float]) -> None:
        ...
        report_writer.write(
            ...
            count_crossovers=metrics["count_crossovers"],
    ...
    def calculate_trend_from_crossover(self, frame: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(len(frame)):
            if not frame["SMA_lower_below_upper"].iloc[i]:
                current_crossover_price = None
                continue
...
if __name__ == "__main__":
    tickers = ["ORCL"]
    crossover_config = CrossoverConfig(lower_sma=20, upper_sma=50, take_profit=0.10, stop_loss=0.05, crossover_length=10, rsi_period=14, rsi_oversold=30, rsi_overbought=70, rsi_underbought=50)
